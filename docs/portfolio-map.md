...
|---|---|---|---|---|
| [Measured model-inference benchmark](https://github.com/yanizhang-yz/robotics-inference-lab/blob/main/experiments/model-benchmarks/results/mobilenet_v3_small.json) | Documented CPU forward-pass benchmarking and latency/throughput analysis | Committed JSON plus a [percentile report](https://github.com/yanizhang-yz/robotics-inference-lab/blob/main/experiments/model-benchmarks/torch-modes-report.md) preserve the CPU device class, model and weights setting, input shapes, warmups, iterations, percentiles, throughput, and limitations; processor/platform and Python/PyTorch/TorchVision versions were not captured | Why batch 8 increased item throughput while increasing per-call latency in this measured run | eligible now — for the CPU benchmark and its bounded analysis, not GPU, server, or policy optimization |
